...
public class SimpleArithmeticLispExprEvaluator
{
    ...
    {
        // TODO PROJECT #5 - Not good, investigate on 9/24-25
      ...
      while (lispExprScanner.hasNext())
        {
            ...
            else //The next character is not an integer
            {
                // grab the character

                character = lispExprScanner.next().charAt(0);
                LispToken token = new LispToken(character);

                //	If you see "(":
                //   	- expect the next character to be an operator
                if(token.operator == '('){ //assuming that token is an operator because it cannot be an operand
                    nextShouldBeOperator = true;
                }

                if(token.operator == ')'){
                    if(nextShouldBeOperator) throw new IllegalArgumentException("Error: Operator was expected.");
                    if(expressionStack.isEmpty()){ throw new InsufficientNumberOfElementsOnStackException("Error: Expression stack is empty");
                    }else{
                        while(!expressionStack.peek().isOperator()){
                            helperStack.push(expressionStack.pop());
                            counter++;
                        }
                    }
                ...
                //	If you see ")":
                //      - if an operator was expected throw an exception ---> Done
                //      - if the expression stack is empty throw an exception --->Done
                ...
                // If you see an operator (“+”, “-“, “*”, or “/”): ---->DONE
